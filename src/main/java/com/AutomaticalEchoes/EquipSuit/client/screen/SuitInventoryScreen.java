...
public class SuitInventoryScreen extends EffectRenderingInventoryScreen<SuitInventoryMenu> {
    ...
    public SuitInventoryScreen(SuitInventoryMenu p_97741_, Inventory p_97742_ ,Component p_97743_) {
        super(p_97741_, p_97742_, Component.nullToEmpty(""));
    ...
    protected void init() {
        ...
        for(int i=0 ;i<4;i++){
            MutableComponent translatable = new TextComponent(Messages.SUIT_NUM[i]);
        ...
        SUIT_NAME = new EditBox(this.minecraft.font,x + 54 ,y - 13 ,56 ,14,Component.nullToEmpty("name"));
        ...
        for (int i=0;i<4;i++){
            TradeOfferButton tradeOfferButton = new TradeOfferButton(x + 1, y + 7 + i * 15, i, Component.nullToEmpty(Messages.PART[i]), SlotUpdatePress, 14, 14) {
                @Override
                public void renderToolTip(PoseStack p_99211_, int p_99212_, int p_99213_) {
                    if (this.isHovered && !buttonClicked) {
                        renderTooltip(p_99211_,Component.nullToEmpty(Messages.PART_NAME[index]),(int)xMouse,(int)yMouse);
                    }
                }
            };
    ...
    public void render(PoseStack p_97795_, int p_97796_, int p_97797_, float p_97798_) {
        ...
        drawString(p_97795_, font,Component.nullToEmpty(Messages.TAG_MODE +Messages.MODE_NAME[EquipSuitClientConfig.CHANGE_MODE.get()])  ,leftPos,topPos+170, 0xFFFFFF);
    }
    ...
    public void resetEditingMessage(){
        ...
        EDITING_MESSAGE.add(Component.nullToEmpty(Messages.TAG_EDITING ));
        EDITING_MESSAGE.add(Component.nullToEmpty(Messages.TAG_SUIT + Messages.SUIT_NUM[((IPlayerInterface) Minecraft.getInstance().player).getFocus()]));
        EDITING_MESSAGE.add(Component.nullToEmpty(Messages.TAG_PART + Messages.PART_NAME[ChangeIndex]));
    }
