...
public class SuitInventoryScreen extends EffectRenderingInventoryScreen<SuitInventoryMenu> {
    ...
    protected void init() {
        ...
        for (int i=0;i<EquipSuit.SIZE;i++){
            TradeOfferButton tradeOfferButton = new TradeOfferButton(x + 3, y + 15 + i * 15, i, new TextComponent(Messages.PART[i]), SlotUpdatePress, 14, 14) {
                ...
                public void renderToolTip(PoseStack p_99211_, int p_99212_, int p_99213_) {
                    if (this.isHovered && !buttonClicked) {
                        renderTooltip(p_99211_,new TextComponent(Messages.PART_NAME[index]),(int)xMouse,(int)yMouse);
                    }
                }
            };
    ...
    public void resetEditingMessage(){
        ...
        EDITING_MESSAGE.add(new TextComponent(Messages.TAG_EDITING ));
        EDITING_MESSAGE.add(new TextComponent(Messages.TAG_SUIT + Messages.SUIT_NUM[((IPlayerInterface) Minecraft.getInstance().player).getFocus()]));
        EDITING_MESSAGE.add(new TextComponent(Messages.TAG_PART + Messages.PART_NAME[ChangeIndex]));
    }
