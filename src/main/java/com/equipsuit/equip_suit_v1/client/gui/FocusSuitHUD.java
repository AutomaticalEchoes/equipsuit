...
public class FocusSuitHUD extends Screen implements Widget, FocusSuitHud {
    private static final Component FOCUS_SUIT_HUD=new TextComponent("focus suit hud");
    ...
    public void render(String s){
        initPos();
        MutableComponent translatable = new TextComponent(s);
        this.renderTooltip(matrixStack,translatable,leftPos,topPos);
        if(time > 0){
            drawString(matrixStack, font,new TextComponent(Messages.TAG_MODE +Messages.MODE_NAME[mode])  ,leftPos+10,topPos-28, 0xFFFFFF);
            time--;
        }
